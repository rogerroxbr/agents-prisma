...
def mock_subgraphs(mocker):
    ...
    MockPubMed = mocker.patch('src.agents.nodes.identify_node.PubMedSubgraph')
    MockScopus = mocker.patch('src.agents.nodes.identify_node.ScopusSubgraph')
    
    mock_pubmed_app = mocker.MagicMock()
    mock_scopus_app = mocker.MagicMock()
    ...
    return (mock_pubmed_app, mock_scopus_app)

...
def synthesize_node(state: PRISMAState) -> PRISMAState:
    ...
    if screening_state and screening_state.decisions:
        for decision_data in screening_state.decisions.values():
            decision = decision_data.get("decision") if isinstance(decision_data, dict) else decision_data
            if decision == "exclude":
                screening_excluded += 1
        
    sought = len(eligible_articles) # In a real scenario, could be higher if some couldn't be downloaded
    ...
    out_path = os.path.join(os.path.dirname(__file__), "..", "outputs", "synthesis_report.md")

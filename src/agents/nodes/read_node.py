...
def read_node(state: PRISMAState) -> PRISMAState:
    ...
    included_ids = [
        aid
        for aid, decision_dict in screen_state.decisions.items()
        if decision_dict.get("decision") == "include"
    ]
    ...
    return {"eligibility": eligibility_state, "progress": 75.0, "phase": "synthesize"}

...
class ThemeGroup(BaseModel):
    theme_name: str = Field(description="Name of the thematic group (e.g. 'Metformin Efficacy', 'Lifestyle Interventions')")
    description: str = Field(description="A short summary of what this theme covers")
    article_ids: List[str] = Field(description="List of article IDs (or DOIs/Titles) belonging to this theme")

class SynthesisResult(BaseModel):
    themes: List[ThemeGroup] = Field(description="List of themes discovered from the articles")
    overall_summary: str = Field(description="A brief paragraph summarizing the overall findings across all included articles.")
...
class SynthesizerAgent:
    def __init__(self, model_name: str = "local-model", temperature: float = 0.0):
        ...
        self.prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert academic researcher synthesizing a systematic review.
    ...
    def synthesize(self, eligible_articles: List[Dict[str, Any]]) -> SynthesisResult:
        ...
        for art in eligible_articles:
            ...
            articles_for_prompt.append({
                "id": art.get("id") or art.get("doi") or art.get("title", "Unknown"),
        ...
        try:
            ...
            return SynthesisResult(
                themes=[ThemeGroup(
                    ...
                    article_ids=[str(a.get("id") or a.get("doi") or a.get("title", "Unknown")) for a in eligible_articles]
                )],
                overall_summary=f"Failed to synthesize automatically. Error: {str(e)}"
            )

    def generate_markdown_report(self, eligible_articles: List[Dict[str, Any]], synthesis: SynthesisResult, project_metadata: Dict[str, Any] = None) -> str:
        """
        ...
        md_lines.append("| ID / Título | População | Intervenção | Comparação | Desfecho | Risco de Viés |")
        md_lines.append("|---|---|---|---|---|---|")

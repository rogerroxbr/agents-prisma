...
class PICOExtracted(BaseModel):
    population: str = Field(
        description="The patient population or problem described in the abstract."
    )
    intervention: str = Field(
        description="The intervention, treatment, or exposure being studied."
    )
...
class ScreeningDecision(BaseModel):
    decision: Literal["include", "exclude"] = Field(
        description="Whether to include or exclude the article based on criteria."
    )
    reason: str = Field(description="A short justification for the decision.")
    confidence_score: float = Field(
        description="Confidence score between 0.0 and 1.0 for this decision."
    )
    extracted_pico: PICOExtracted | None = Field(
        description="The extracted PICO elements, if applicable."
    )
...
class ScreeningAgent:
    ...
    def load_criteria(self, criteria_file: str = "config/prisma_criteria.json"):
    ...
    def _build_prompt(self, article: dict[str, Any]) -> str:
        ...
        inclusion_list = screening_criteria.get(
            "inclusion_criteria", self.criteria.get("inclusion_criteria", [])
        )
        exclusion_list = screening_criteria.get(
            "exclusion_criteria", self.criteria.get("exclusion_criteria", [])
        )
...
"""
    ...
    def _evaluate_article(self, article: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            print(
                f"[SCREENING] LLM Error evaluating article '{article.get('title')[:30]}...': {e}"
            )
    ...
    def _fallback_evaluate_article(self, article: dict[str, Any]) -> dict[str, Any]:
        ...
        if any(kw in title for kw in exclude_keywords):
            return {
                "decision": "exclude",
                "reason": "Fallback: Exclusion keyword in title",
                "score": 0.1,
                "pico_extracted": None,
            }
        ...
        return {
            "decision": "include",
            "reason": "Fallback: Default include",
            "score": 0.5,
            "pico_extracted": None,
        }

    def screen_batch(
        self, articles: list[dict[str, Any]], batch_size: int = 50
    ) -> list[dict[str, Any]]:
        """Screen a batch of articles using LLM-based filtering."""
        print(
            f"[SCREENING] Screening batch of {min(len(articles), batch_size)} articles with LLM..."
        )
        ...
        print(
            f"[SCREENING] Batch complete: {len(included)} included, {len(excluded)} excluded"
        )

        return included + excluded


__all__ = ["PICOExtracted", "ScreeningAgent", "ScreeningDecision"]

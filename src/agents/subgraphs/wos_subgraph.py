"""Web of Science Subgraph for executing WoS searches."""
...
from src.agents.nodes.query_wos_node import query_wos_node
from src.agents.subgraphs.state import SearchSubgraphState


class WosSubgraph:
    """WoS subgraph following the Diamond Pattern (Query -> Extract)."""
    ...
    def _build_graph(self):
        ...
        self.workflow.add_node("query_wos", query_wos_node)
        self.workflow.add_node("extract_metadata", extract_metadata_node)

        self.workflow.add_edge(START, "query_wos")
        self.workflow.add_edge("query_wos", "extract_metadata")

...
class ScopusTool:
    ...
    def search(
        ...
        """
        self.logger.info(f"Searching Scopus for query: {query!r}", extra={
        ...
        for attempt in range(1, retry_attempts + 1):
            try:
                ...
                if response.status_code != 200:
                    error_msg = f"Scopus API HTTP {response.status_code}: {response.text[:200]}"
                ...
                error_msg = f"Scopus API network error (attempt {attempt}/{retry_attempts}): {str(e)}"
                self.logger.warning(error_msg)
                last_error = e
    ...
    def _parse_results(self, json_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        if "result" not in json_data or "item" not in json_data["result"]:
            self.logger.info(f"No results found for query", extra={"query": ""})
            return results
        ...
        for item in json_data["result"]["item"]:
            ...
            if pub_date:
                try:
                    result["pub_date"] = pub_date.split("-")[0]  # Keep YYYY-MM-DD format
            ...
            if "abstract" in item:
                ...
                if abs_data:
                    ...
                    self.logger.debug(f"Abstract extracted for: {result['title'][:50]}...")
